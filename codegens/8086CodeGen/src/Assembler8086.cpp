 ...
 * 
 * Finally after a short while of working on the assembler I thought it may be more appropiate to have an array determine
 * how instructions should be generated, this would be far more efficient but may be hard to get right, so I am avoiding that for now.
 * 
 * If I ever did do this the array could hold information on every instruction, meaning only a method for each type of instruction would be required, 
 * or maybe even just one method.
 * 
...
void Assembler8086::get_modrm_from_instruction(std::shared_ptr<InstructionBranch> ins_branch, char* oo, char* rrr, char* mmm, bool oo_default_use_reg_no_addr_mode)
{
    unsigned int number;
    ...
    if (oo_default_use_reg_no_addr_mode)
    {
...
int Assembler8086::get_instruction_size(std::shared_ptr<InstructionBranch> ins_branch)
{
    ...
    {
        get_modrm_from_instruction(ins_branch, &oo, &rrr, &mmm, false);
...
void Assembler8086::generate_mov_imm_to_mem(int opcode, std::shared_ptr<InstructionBranch> instruction_branch)
{
    ...
    // Get the modrm for this instruction
    get_modrm_from_instruction(instruction_branch, &oo, &rrr, &mmm, false);
...
void Assembler8086::write_modrm_offset(std::shared_ptr<OperandBranch> branch)
{
    switch (oo)
    {
